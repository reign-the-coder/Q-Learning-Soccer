...
class Player:
    ...
    def __init__(self, x, y, has_ball, player_id=None):
...
class World:
    ...
    def map_player_state(self):
        ...
        key = player_a.player_id if player_a.has_ball else player_b.player_id
    ...
    def check_collision(self, new_pos, moving_player, other_players):
        ...
        for oplayer_id in other_players:
            other_p = self.players[oplayer_id]

            if new_pos.x == other_p.x and new_pos.y == other_p.y:

                if self.commentator:
                    print ('{} collided with {}'.format(moving_player.player_id, other_p.player_id))
                ...
                if new_pos.has_ball:
                    ...
                    if self.commentator:
                        print ("{} steals from {}".format(other_p.player_id, moving_player.player_id))
    ...
    def check_goal(self, moving_player, player_order):
        ...
        goal = False
        other_players = set(player_order) - set(moving_player.player_id)
        r = {k: 0 for k in player_order}

        if moving_player.x == self.goals[moving_player.player_id] and moving_player.has_ball:

            if self.commentator:
                print ("{} scored a goal!!".format(moving_player.player_id))

            goal = True
            r[moving_player.player_id] = self.goal_r[moving_player.player_id]

            for oplayer_id in other_players:
                r[oplayer_id] = -self.goal_r[moving_player.player_id]

        else:
            other_goal = {oplayer_id: moving_player.x == self.goals[oplayer_id] and moving_player.has_ball
                          for oplayer_id in player_order}

            if sum(other_goal.values()) > 0:

                if self.commentator:
                    print ("{} scored an own goal!!".format(moving_player.player_id))
                    print (self.goal_r[moving_player.player_id])
    ...
    def move(self, a):
        ...
        for player_id in player_order:
            ...
            other_players = set(player_order) - set(player_id)
            new_pos.update_state(moving_player.x, moving_player.y, moving_player.has_ball)
            action = self.actions[a[player_id]]
...
def construct_env(world):
    ...
    player_a = Player(x=2, y=0, has_ball=False, player_id='A')
    player_b = Player(x=1, y=0, has_ball=True, player_id='B')
